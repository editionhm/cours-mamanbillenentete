        ...
        <div className="min-h-screen bg-slate-50/50">
            <div className="container py-24 md:py-32 px-4 mx-auto space-y-20">
                ...
                <div className="text-center max-w-3xl mx-auto space-y-6">
                    <Badge className="mb-4 px-4 py-1 text-sm bg-primary/10 text-primary hover:bg-primary/20 border-none transition-all">
                        Tarifs 2025-2026
                    </Badge>
                ...
                <div className="space-y-12">
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto items-start">
                        {individualRates.map((offer) => (
                            <Card key={offer.name} className={`flex flex-col relative transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 rounded-3xl overflow-hidden border-slate-100 ${offer.highlight ? 'bg-slate-900 text-white shadow-xl scale-105 z-10 ring-4 ring-primary/20' : 'bg-white hover:border-slate-200'}`}>
                                {offer.highlight && (
                                    <>
                                        <div className="absolute top-0 inset-x-0 h-2 bg-gradient-to-r from-primary to-secondary" />
                                        <div className="absolute top-4 right-4 animate-pulse">
                                            <Badge className="bg-gradient-to-r from-primary to-secondary text-white border-none shadow-lg px-3 py-1">
                                ...
                                <CardContent className="flex-1 p-8 pt-2">
                                    <div className="flex flex-col mb-8 items-center text-center">
                                        <div className="flex items-baseline gap-1">
                                            <span className={`text-4xl font-extrabold font-sans ${offer.highlight ? 'text-white' : 'text-primary'}`}>{offer.price}</span>
                                        </div>
                                    ...
                                    <ul className="space-y-3">
                                        {offer.features.map((feature) => (
                                            <li key={feature} className="flex items-start gap-3 text-sm">
                                                <div className={`p-1 rounded-full mt-0.5 shrink-0 ${offer.highlight ? 'bg-white/20' : 'bg-primary/10'}`}>
                                                    <Check className={`h-3 w-3 ${offer.highlight ? 'text-white' : 'text-primary'}`} />
                                                </div>
                ...
                <div className="max-w-4xl mx-auto mt-20">
                    <Card className="bg-gradient-to-br from-secondary/5 to-primary/5 border-none shadow-lg overflow-hidden rounded-3xl relative">
                        <div className="grid md:grid-cols-2 gap-8 p-8 md:p-12 items-center">
                            <div className="space-y-6">
                                <div className="space-y-2">
                                    <Badge className="bg-secondary text-white border-none mb-2">Cours Collectifs</Badge>
                            ...
                            <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 text-center space-y-6">
                                <div>
                                    <span className="text-5xl font-extrabold text-secondary block mb-1">{groupRate.price}</span>
                                    <span className="text-slate-500 font-serif">{groupRate.period}</span>
                                </div>
                                <Button className="w-full rounded-full h-12 text-lg bg-secondary hover:bg-secondary/90 text-white">
                                    {groupRate.action}
                                </Button>
                            </div>
                        </div>
                ...
                {/* Contact CTA */}
                <div className="bg-slate-900 rounded-3xl p-12 text-center text-white space-y-8 max-w-5xl mx-auto shadow-2xl relative overflow-hidden">
